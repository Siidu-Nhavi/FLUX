...
async function signup(req, res, next) {
  try {
    ...
    return sendSuccess(res, auth, auth.session ? 'Account created.' : 'Check your email to confirm the account.', 201);
  } catch (error) {
    return next(error);
  }
}
...
async function resetPassword(req, res, next) {
  try {
    ...
    return sendSuccess(res, { passwordUpdated: true }, 'Password updated successfully.');
  } catch (error) {
    return next(error);
  }
}
